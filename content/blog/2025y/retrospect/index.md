...
내가 맡은 업무  영역이 아닌 다른 서비스의 업무를 경험할 기회를 달라고 팀장님께 자주 어필했고, 이슈나 장애가 발생하면 새벽에도 나타나 어깨너머로 배우며 도움을 주기도 했다.  
다행히 스윙 바이크에 신규 IoT 모델을 추가하는 작업을 맡게 되면서 IoT 인프라를 경험하고 이해할 수 있었다.  
  
스윙 바이크 어드민을 궤도에 올리고 IoT 파이프라인, 인프라 작업까지 경험하면서부터 다른 서비스와 회사에 조금씩 눈이 갔다.  
...
2. **문제를 해결하는 방법**
   ...
   - 해결해야 할 본질적인 문제를 먼저 확실하게 이해하려 노력하는 습관이 생겼다. 새로운 문제가 나타나면 왜 발생했는지, 지금 해결해야 하는 건지, 애초에 진짜 문제가 맞는지, 가장 빠르게 해결할 방법은 뭔지를 따져보게 됐다.
...
4. **책임감**
   - 서비스를 혼자 맡다 보니 자연스럽게 프로덕트에 대한 책임감이 생겼다.
   - 빠른 개발 속도 탓에 기술 부채가 쌓였지만, 코드 퀄리티나 아키텍처 개선, 결제 에러 수정 등을 책임감(애정?)으로 주말이나 공휴일에 개인 시간을 할애하며 처리할 수 있었다.
   - 이런 개선 내용을 팀에 공유하고 마무리까지 해내면서 보람을 많이 느꼈다.
...
가장 먼저 한 일은 실무에서 장애를 해결하면서 깊게 파보지 못했던 부분들을 보완하기 위해 딥다이브하는 것이었다.  
...
4. **E회사** : 평소 관심 있던 도메인이었고, 면접에서부터 도전적인 업무를 진행할 예정이라고 말씀해주셔서 입사를 결정했다.

퇴사라는 선택이 무모할 수도 있었지만, 4개월간 열심히 준비한 보람이 있었다.  

# 런닝

올해는 회사 일이 바빠서 평일에 많이 못 뛰었지만 회사에서 주말 스터디를 만들어서 팀원들을 런닝하자고 꼬셔서 주말에 1번이라도 꾸준히 뛴 게 도움이 됐다.  

![](./running1.jpg)
![](./running2.jpg)

마라톤 대회는 5번 정도 나갔는데, 기록 사진을 모두 캡처해놓지 않아서 그나마 하프 대회 기록을 올린다.  
...
아직은 개발 지식이 없는 사람이 일정 수준 이상의 복잡도를 가진 애플리케이션을 만드는 데 한계가 있다고 생각한다.  
...
회사에서 에이전틱 코딩 TF 리드를 맡게 됐는데, 이 기회에 더 열심히 할 동기도 생겼다.  
