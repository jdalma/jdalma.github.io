...
1. **SDUI의 성능 병목 해결**
    -  앱 배포에 유연하게 대응하기 위해 UI 정보를 서버에서 관리
    -  이때 레디스를 활용하여 빠른 응답을 도모했지만 서버 <-> 레디스 네트워크 병목으로 인해 API 응답 속도 저하 발생
    -  1차: Caffeine, 2차: 레디스, 3차: DB 조회
    -  **캐시 무효화 전략** : 백오피스에서 캐시 삭제 가능, 캐시된 데이터 불일치 방지를 위해 하루에 한 번 사전 캐싱

## 생각해볼 점

1. 인메모리, 레디스를 사용하는 경우 캐시 관통은 어떻게 해결할까?
    - 키 기준으로 뮤텍스나 락을 이용하여 한 요청만 실제로 데이터를 갱신하는 역할을 하도록
    - 존재하지 않는 키에 대한 요청도 캐시 관통이 발생할 수 있기에 `없음` 결과를 캐싱해두는 식으로 재조회 폭탄을 막기


# 올리브영 : 비동기 요청-응답 패턴으로 풀어낸 발주 서비스 개발기
