...
> 그리고 작은 프로그램, 타입에 대한 이해 수준이 낮게 요구될 때에는 동적 타입 언어가 더 유리하다.  
  
정적 타입 언어가 유용하지만 **타입 검사기가 `거부`라고 오판하는 경우가 반드시 존재한다.**  
개발자들은 타입 검사기의 오판을 줄이고 정적 언어를 잘 사용하기 위해 여러 경우에 대한 다형성,가변성에 대한 이해와 타입 검사기와 친해지도록 노력하여야 한다.  
이 책이 그 노력에 대해 도움을 준다.  
  
와닿았던 부분은 제네릭 가변성에 대한 이해가 없을 때 이게 왜 안되는지 몰랐던 부분들을 깨닫게 도와줬다. [PECS : producer-extends, consumer-super](https://github.com/jdalma/footprints/blob/main/effective-java/item31_%ED%95%9C%EC%A0%95%EC%A0%81%20%EC%99%80%EC%9D%BC%EB%93%9C%EC%B9%B4%EB%93%9C%EB%A5%BC%20%EC%82%AC%EC%9A%A9%ED%95%B4%20API%20%EC%9C%A0%EC%97%B0%EC%84%B1%EC%9D%84%20%EB%86%92%EC%9D%B4%EB%9D%BC.md#pecs--producer-extends-consumer-super)를 외워서 사용했는데 왜 저렇게 제한해야 하는지 더 깊은 이해를 할 수 있도록 도와주었다.    
  
1. 서브타입에 의한 다형성
   1. 이름에 의한 서브타입
   2. 구조에 의한 서브타입
   3. 추상 메서드가 필요한 이유
나는 주로 자바와 코틀린을 사용하기 때문에 책의 초반에 나오는 내용들을 따라가기가 쉬웠다.  
