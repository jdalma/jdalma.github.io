...
하지만 Spring webflux와 R2DBC가 도메인을 이해하는데 걸림돌이 될 정도는 아니며 제공되는 교안이 상당히 고품질이라는 것이다.  
추후에 예제 코드와 교안만 다시 보아도 이해하는데 문제가 없을정도이다.  
그래도 헥사고날 아키텍처는 이 강의를 보기전에 대략적으로 이해하고 보는것을 추천한다. 그렇지 않으면 패키지 구조나 왜 이런 추상화와 구현체가 생기는지 이해할 수 없어 넋놓고 따라치기만 하는 스스로를 볼 수 있다..  
사전에 이 [강의](https://www.inflearn.com/course/%EC%9E%90%EB%B0%94-%EC%8A%A4%ED%94%84%EB%A7%81-%ED%85%8C%EC%8A%A4%ED%8A%B8-%EA%B0%9C%EB%B0%9C%EC%9E%90-%EC%98%A4%EB%8B%B5%EB%85%B8%ED%8A%B8/dashboard)를 보길 추천한다.  
  

## Java/Spring 테스트를 추가하고 싶은 개발자들의 오답노트

이 강의가 새로 등록된 것을 보고 테스트 코드는 단방향으로 학습하는 것에 한계가 있고 실제로 적용하면서 많이 배운다고 생각하고 있었기에 구매할 생각은 없었다.  
하지만 [수강평](https://inf.run/FcsUb)과 커리큘럼에 헥사고날 아키텍처가 있어서 구매해보았다.  
([견고한 결제 시스템 구축](https://www.inflearn.com/course/%EA%B2%AC%EA%B3%A0%ED%95%9C-%EA%B2%B0%EC%A0%9C-%EC%8B%9C%EC%8A%A4%ED%85%9C-%EA%B5%AC%EC%B6%95?srsltid=AfmBOooVYgoCpuSqsins2KXxboQ_iBZVndJoB9L7fa6OFB39qOdCTztw) 강의를 들으면서 헥사고날 아키텍처 때문에 고생하고 있었다.)  
...
얼마전에 올라온 인프런의 [토비님 강의](https://www.inflearn.com/course/%ED%86%A0%EB%B9%84%EC%9D%98-%EC%8A%A4%ED%94%84%EB%A7%816-%EC%9D%B4%ED%95%B4%EC%99%80-%EC%9B%90%EB%A6%AC#)가 토비의 스프링 1권에 대한 내용을 설명한다고 하셔서 학습해보았다.    
...
얄팍한 지식으로 대충 지레직잠하고 있던 내용들도 있었고, 깨닫게 되는 내용도 많았어서 좋았다.  
...
객체지향 프로그래밍이라는 크고 두루뭉술한 단어에 대한 스스로의 주관적인 생각과 윤곽을 가지기에 좋은 책이라는 생각이 들었다.  
...
그리고 코틀린 다운 코드는 어떻게 작성하는지, `reified`가 무엇인지 알 수 있었다.  
  
코틀린 기본 정도만 알고 있으면 충분히 학습할 수 있는 내용들이라서 코틀린과 코루틴에 대한 인사이트를 얻고 싶다면 추천하고 싶다.  
스터디 내용은 [여기서](https://github.com/jdalma/footprints/blob/main/%EC%8A%A4%ED%84%B0%EB%94%94/%EC%BD%94%ED%8B%80%EB%A6%B0%20%EB%94%94%EC%9E%90%EC%9D%B8%ED%8C%A8%ED%84%B4.md)확인할 수 있다.  
...
MySQL을 사용하는 개발자라면 필수로 읽어야 할 책이라고 생각한다.  
...
6. `일급 컬렉션`에 대한 내용을 [넥스트스텝 코틀린 교육](https://jdalma.github.io/2023y/kotlinCleanCode/)에서 접했을땐 코틀린에서 지원하는 컬렉션 공통 함수들을 사용하기 불편하다고 생각해서 와닿지 않았었는데 이 책을 통해 잘못이해하고 있었다는 것을 느꼈다.  
  
인상 깊은 내용들은 이 정도로 정리할 수 있겠다. 중후반부 까지는 좋은 코드를 작성하는 방법, 설계에 대한 강조, 모델의 개념들에 대해 설명했다.  
([엘레강트 오브젝트](https://jdalma.github.io/2023y/bookReview/bookReview/#%EC%97%98%EB%A0%88%EA%B0%95%ED%8A%B8-%EC%98%A4%EB%B8%8C%EC%A0%9D%ED%8A%B8)가 자주 떠올랐다.)  
...
결국 **지금 돈 벌기**와 **지금 돈 쓰기**를 고민하는 것이다.  
...
[옮긴이의 글들](https://yozm.wishket.com/magazine/@bettercoder/)도 참고하자.
...
이런 고민과 결정은 **책임감**과 **장인 정신**을 지키기 위한 자신의 양심과 자랑스럽지 않은 일을 하지 않으려는 마음에 의존한다.  
...
이때는 학습한 이론과 패턴이 아닌 결정 사항에 영향을 끼치는 **가치**와 **원칙** 이 존재한다.  
- 가치는 코드의 복잡성을 낮추고 가독성을 중시하며, 유연성있는 코드를 작성하는 클린 코드의 **가치**이다. 
- 원칙은 가치처럼 언제나 적용되는 것은 아니지만 때로는 직접 적용하기 어려운 가치와 적용법은 명확하지만 조금은 지엽적인 패턴 사이의 가교 역할을 하는 것이 **원칙**이다.
...
책 읽기 전에 [유퀴즈에 출연한 영상](https://www.youtube.com/watch?v=hEu_16312Xw)도 시청하는 것도 좋다.  
...
이 기저에는 새벽 시간을 통해 `내가 주도하는 시간`을 꾸준히 만들어낸 것이다.  
저자는 살면서 무언가를 쉽게 얻은 적이 없다고 얘기한다. 사람들이 피하는 험난한 길을 걸으며 인내를 기르고, 힘듦에서 즐거움을 찾는 습관을 갖게 되었다고 한다.  
  
- 새벽 기상은 시간이 없다는 핑계를 댈 수 없게 만든다. 새벽 시간은 `내가 주도 하는 시간`, 그 밖의 시간은 `운명에 맡기는 시간`이다.
...
[너진똑 - 카뮈 [이방인]](https://www.youtube.com/watch?v=Hdf35IzBpIU&ab_channel=%EB%84%88%EC%A7%84%EB%98%91NJTBOOK)의 영상을 보고 내용이 궁금하여 읽어보았다.  
[허무주의](https://ko.wikipedia.org/wiki/%ED%97%88%EB%AC%B4%EC%A3%BC%EC%9D%98)와 [염세주의](https://ko.wikipedia.org/wiki/%EC%97%BC%EC%84%B8%EC%A3%BC%EC%9D%98)에 가까워 보이는 주인공이 삶을 대면하는 자세를 볼 수 있었다.  
...
> 공부하고 싶어도 가정형편 때문에 못 하고, 안락한 주거환경에서 편안한 생황을 하고 싶지만 항상 불안에 휩싸여 낮은 삶의 질을 경험해야 하고,  
...
상대적으로 무엇인가를 더 가진 자와 비교하면서 발생하는 `불평등`에 대한 장자크 루소의 개인적인 생각을 전개한다.  
...
다른 사람들보다 더 부유하다거나, 더 존경을 받는다거나, 권력을 더 가지고 있다거나 또는 타인을 복종하게 만든다거나 하는 `특권`들에 의해 성립되는 두 번째 불평등에 대해 소개한다.  
이 불평등의 기원은
...
> "`혼돈`만이 우리의 유일한 지배자 이며, `혼돈`은 우리의 그 무엇에도 관심이 없단다."  
...
[데이비드 스타 조던](https://en.wikipedia.org/wiki/David_Starr_Jordan)은 19세기에 스탠포드 대학교의 창립 총장으로 재임했었던, 어류학자로서 지대한 영향을 끼친 인물이다.  
...
4. **20세기 초반, 과학의 탈을 쓰고 퍼져나갔던 거짓된 신념 `우생학`의 기여를 한 전도사**
5. **`우생학`적 목표를 이루기 위해 전쟁을 반대하는 평화주의자**
6. **`우생학`으로 인한 강제적인 불임 시술 지지**
