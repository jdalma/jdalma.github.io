...
여러 프로젝트의 서비스를 동시에 만지다 보면, 각 세션이 서로 대화하면서 컨텍스트를 같이 쌓고 협력하게 만들고 싶어진다.  
예를 들어 주문 서버와 결제 서버를 함께 작업해야 하는 경우 API 작업, 컨텍스트, 의사결정을 매번 각 세션에 복사,붙여넣기하고, 주문 서버에서 분석한 내용을 파일로 저장해 결제 서버에 그 경로를 다시 붙여넣는 식이었는데, 이 과정이 꽤 피곤했다.  
  
  | 기능 | Claude Code agent teams | OMC `/team` | OMC `/omc-teams` | my-team |
  |---|---|---|---|---|
  | **① 워커 실행 방식** | 독립 Claude Code 세션(논리적) | hybrid — claude 멤버=논리 세션 / CLI 워커=독립 tmux 프로세스 | 전원 독립 tmux CLI 프로세스 | 전원 독립 tmux CLI 프로세스 |
  | **② 통신** | native mailbox(자동 전달, peer-to-peer) | 멤버별 라우팅 — native 멤버=`SendMessage` / CLI 워커=파일 inbox | 파일 inbox/mailbox + tmux 알림 | 파일 spool→mailbox(pull) + tmux 알림 |
  | **③ 프로바이더** | Claude 전용 | Claude + Codex + Gemini | claude / codex / gemini | CLI 무관(무엇이든 pane) |
  | **④ 워커별 cwd / 워크스페이스** | ❌ 모두 리더 cwd 공유(파일만 분담) | △ worktree 모드 시 같은 repo의 워커별 worktree(이름 자동) · 다른 프로젝트 ❌ | ❌ 단일 workspace | ✅ 워커별 임의 cwd 직접 지정(서로 다른 프로젝트 가능) |

omc의 omc-teams가 tmux와 mailbox로 통신하는 걸 보고, 그 구조를 거의 그대로 가져온 뒤 원하는 프로젝트와 모델을 config 파일로 지정할 수 있게 만들어봤다.  
(감사합니다 omc!)  
  
대신 리더, 메시지 전송,리트라이,중복 제거 같은 기능은 덜어내고 최소한만 남겼다.  

```
~/.my-team/sessions/<team>/
│
├─ manifest.json          팀 구성 (워커 명단·설정)
├─ events.jsonl           팀 전체 메시지 타임라인 (보낼 때마다 한 줄)
├─ shutdown.json          종료 신호
│
├─ incoming-spool/        임시 우편함 (받는 사람 칸으로 나뉨)
│   ├─ A/
│   │  └─ <편지>.json      A가 받은 편지 (한 통 = 파일 하나)
│   └─ B/
│      └─ <편지>.json      B가 받은 편지
│
├─ mailbox/               받은편지함 (각자 꺼내온 편지)
│   ├─ A.json
│   └─ B.json
│
├─ archive/               주고받은 기록 (한 줄에 한 통)
│   ├─ A.jsonl
│   └─ B.jsonl
│
└─ workers/               워커별 정보 (워커별 동일 구성)
    ├─ A/
    │   ├─ AGENTS.md          이 워커의 행동 지침
    │   ├─ heartbeat.json     살아있다는 신호
    │   ├─ status.json        지금 상태
    │   ├─ .ready             준비 끝 표시
    │   └─ shutdown-ack.json  종료 확인
    └─ B/ ...
```

여러 프로젝트를 엮어서 작업하고 싶을 때 요긴하게 쓰고 있다.

### admin-agent

어드민을 자연어로 쓸 수 있게 embabel로 어드민 유즈케이스를 구현하고 있다. 써보니 사용자와 모델 사이의 모호함을 꽤 잘 풀어주는 프레임워크 같다.  
  
사용자 프롬프트에서 어떤 유즈케이스인지를 판별하면 AgentProcess가 만들어진다.  
`AgentProcess`는 사용자의 발화 한 건을 정의된 워크플로우의 실행 중인 인스턴스로 만든 것이다. 즉, 워크플로우의 상태와 목표에 도달할 수 있는 방법들을 보유한 클래스이다.  
  
AgentProcess의 대표적인 구성 요소로는, 

1. `Blackboard` : 워크플로우의 상태를 관리하고, tick (도구 실행) 마다 저장되는 데이터들을 보관한다.
2. `Planner` : GOAP 플래너, 목표에 도달하기 위해 사용할 수 있는 액션 (도구)들을 탐색하며, Blackboard에 저장된 상태와 데이터들을 참고해서 GOAP를 계획한다.
3. `Agent` : 각 작업 흐름을 에이전트라고 칭한다.
  
좀 더 파악되면 딥다이브 글로 따로 정리할 생각이다.  
지금까지의 느낌은, 사용자와 모델 사이의 모호한 지점을 명시적으로 선언하도록 도와주는 프레임워크라고 느꼈다.  
복잡한 워크플로우도 해결할 수 있을 만큼 쓸모 있어 보인다.  

### 느낀 점

3개월 동안 3.79 Bil 정도의 토큰을 사용하면서 많은 것을 경험한 것 같다.  
  
1. 모놀리식 프로젝트를 MSA로 분리하는 바이브 코딩
2. 사내에서 사용하는 클로드 코드 플러그인 개발
3. 개인 클로드 코드 플러그인 개발
4. AI 에이전트 바이브 코딩
  
여러 작업들을 토큰을 태워가면서 느낀 점은 AI를 어디까지 믿고 신뢰할지? 내가 모든 작업들을 이해하고, 알고 있지 않아도 테스트 코드로 모두 예상할 수 있는 범위인지? 테스트 코드는 진짜 정확한 기능을 테스트하고 있는지 확인해야할지? 이런 의문들을 해결할 수 있는건 하네스인지? 하네스를 얼마나 갈고 닦아야할지? 이런 생각들이 꼬리를 물었다.  
  
바이브코딩을 시작하면서 많은 부분을 맡기면서 진행하다보니 나중에 검수하는 시간이 더 많이 소요된 것 같다고 느꼈다.  
그래서 아래의 규칙을 만들었다.  
  
1. 태스크는 테스트 코드로 검증할 수 있고 작업자가 인지할 수 있을 정도의 크기까지 분할한다.
2. 세션의 연속성을 보장하기 위해 작업 내용을 기록하고, 로드하는 것을 습관으로 만든다.
3. AI에게 개발은 맡기 돼, AI가 개발하는 내용에 대해 이해하고 명령한다.
  
이런 규칙을 지키기 위해 나만의 워크플로우를 만들어서 사용 중이다.  
개발자의 역량 중 AI 활용 능력이 큰 범위를 차지하게 될 것 같은데, 이런 노력이 꾸준히 필요하다고 생각한다.  
...
도메인과 정책 지식을 기반으로 답변해주는 AI 에이전트를 만들고 싶었다.  
플러그인으로는 한계가 있다고 느껴서 챗봇 형태로 제공하려고 SDK로 개발을 시작했는데, 막상 어떤 문제를 풀어야 할지가 분명하지 않았다.  
  
지금 생각해보면, 풀어야 할 문제부터 잡았어야 했는데 SDK로 뭔가 만들고 싶다는 마음이 앞서서 방법론에만 매달렸던 것 같다.  
그래서 요즘은 이 작업에 따로 손대지 않고 있다.  

### 요즘 시도하는 것들

<h4>어드민을 자연어로 쓰기</h4>

문제거리를 찾으려고 주변을 살펴보다가, 어드민 조작이 익숙하지 않아서 발생하는 운영 이슈가 꽤 잦다는 걸 알게 됐다.  
원인은 개발팀과 운영팀 사이의 소통 문제처럼 보였는데, 소통 자체를 줄일 수 있는 방법으로 어드민을 자연어로 쓰는 그림이 떠올랐다.  
  
상품 프로모션을 등록할 때 어드민에서 직접 만지다 보면 빠뜨리는 단계가 생기기 마련이다. 자연어로 워크플로우를 강제하고 빠진 게 있으면 작업자한테 알려주는 챗봇이 있으면 쓸모가 있겠다 싶었다.  
요즘은 [embabel](https://github.com/embabel)로 기술 검증을 해보고 있다.  
  
지금은 커머스 운영팀과 가까이 일하고 있진 않지만, 다음 작업에서 PM까지 겸하게 될 것 같아서 그때는 실제로 써먹을 수 있지 않을까 한다.  

<h4>바이브코딩 워크플로우 개선하기</h4>

모놀리식 애플리케이션에서 결제 도메인을 떼어내는 작업을 바이브코딩으로 진행하면서 몇 가지를 느꼈다.  

1. ADR, TODO, PLAN, SPEC 같은 문서를 신경 써서 써도, 작업자가 놓친 건 AI도 똑같이 놓친다.
2. AI는 문제를 발산하는 쪽엔 강해서, 그걸 수렴시킬 수 있을 만큼은 사람이 기술 지식을 가지고 있어야 한다.
3. AI는 코드를 빨리 뽑아내려고 해서, 내 인지를 벗어나는 코드가 슥 만들어진다.
4. 태스크 단위가 너무 크면 안 된다. 1M 컨텍스트를 믿기보다는, 컨텍스트의 절반 정도만 차지하는 크기로 쪼개는 게 마음 편하다.
5. 아무리 꼼꼼하게 설계하고 계획해도 놓치는 건 꼭 나온다.

이걸 보완하려고 `takeover >> plan >> tdd >> handoff` 흐름을 만들었다.  
중요하게 본 건 두 가지다.

1. 지금 내가 어떤 단위로 일하고 있는지를 스스로 알고 있어야 한다.
   - `plan`으로 초기 태스크를 등록한다.
   - `tdd`로 쪼개진 태스크를 하나씩 처리한다.
   - 진행하다가 다른 태스크가 갈라져 나오면, 새 태스크로 뺄지 지금 세션에서 끝낼지를 그때그때 정한다.
   - `tdd` 작업의 단위는 작업자가 꼭 인지하도록 노력한다.
2. 세션이 끊겨도 맥락을 이어가려면 인수인계 방식을 정해둬야 한다. [Claude 세션 간 Context Handoff: 맥락을 잃지 않는 4계층 전략](https://codex.epril.com/claude-session-context-handoff-4-layer-strategy) 글을 참고했다.
   - `handoff`로 작업 상태를 적어두고, `takeover`로 그 내용을 다시 컨텍스트에 올린다.
   - 태스크는 트리로 관리하고, 각 태스크의 구현 내용은 문서로 남겨서 추적 가능토록 한다.

<h4>LLM wiki 관리하기</h4>

[LLM wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)를 보고 로컬에 wiki를 굴리고 있다.  
기록용으로만 두는 게 아니라, 로컬 MCP랑 검색 엔진을 붙여서 의사결정을 대신 시키거나, 예전에 비슷한 작업을 해둔 게 있는지 찾는 용도로 쓰고 있다.  
  
Claude Code 훅으로 프롬프트를 한 번 평가해서, wiki를 볼 만한 질문이면 프롬프트에 지시문을 끼워 넣어 모델이 wiki를 뒤지게 한다.  
관리까지 다 자동화해버리면 wiki 품질이 금방 떨어질 것 같아서, 의사결정에 영향을 주는 문서는 내가 직접 읽고 거른다.  
로컬 MCP 서버가 과해지지 않게 신경 쓰는 중이고, wiki 문서를 다듬는 일 자체가 꽤 재미있다.  

### 느낀 점

바이브코딩이 쓸 만한 일을 맡고 있는 게 다행이다 싶다. 직접 적용해보면서 AI가 어디서 막히는지 체감해보고, 내 일에 어떻게 녹일지 고민해보는 게 지금 시기에 제일 남는 장사라는 생각이 든다.  
명확하게 지시하지 않은 부분까지 AI가 알아서 해줄 거라고 믿고 기대면 안 된다. 나보다 똑똑한 건 맞지만, 그 똑똑함을 어디에 쓸지는 내가 알려줘야 한다.  
내 하네스를 계속 만들고 다듬으면서, 좀 실패해도 또 해보는 걸 즐기려 한다.  
