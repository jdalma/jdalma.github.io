...
사내의 에이전틱 코딩 TF를 도맡게 됐다. 회사에서 원하는 방향과 내가 현재 개인적으로 관심가지고 있는 방향이 동일하여 주최한 스터디가 TF로 변하게 됐다.  
그래서 요즘 `AI TPM`, `에이전틱 코딩 컨벤션`에 대해 관심가지고 있다.  
  
AI 관련 기술들이 쏟아지는 상황에서 백엔드 개발자로서 가장 의미있는 작업이 뭘까 고민을 하다보니 에이전틱 코딩에 관심가지게 된것 같다.  

1. 클로드 코드가 어떤 에이전트/스킬/훅을 의도한대로 호출하는지 확인할 수 있는 오케스트레이션 모니터링 툴
2. 도메인 지식, 코드베이스 기반 상호 의존성 및 연관관계에 대한 컨텍스트 관리 방법
   - CLAUDE.md 중첩하기, 라우팅 레이어 만들기, 서브에이전트가 실행될 때 해당 에이전트 전용 컨텍스트를 삽입하기, 특정 상황에는 에이전트 팀즈를 실행하기 등
3. 테스트 가능한 방법
   - 중복된 CLAUDE.md를 읽었는가?
   - 의도한 스킬이 트리거됐는가?
...
위의 문제를 해결하기 위해 앤트로픽의 공식 문서부터 차츰 읽어가고 있다.

- [PLANS.md](https://developers.openai.com/cookbook/articles/codex_exec_plans)
- [AI 모범사례에 대해](https://bits.logic.inc/p/ai-is-forcing-us-to-write-good-code)
- [toneylee - openai harness](https://tonylee.im/ko/blog/openai-harness-engineering-five-principles-codex/)
