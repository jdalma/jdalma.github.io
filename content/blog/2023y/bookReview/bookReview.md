...
도메인 주도 설계에 대해 공부하기전에 읽어보기를 추천받았다.  
DDD를 이루는 개념과 원리를 공부하기 보다는 실무 도메인(문제를 해결하려는 영역)에 대한 이해를 강조한다.  
(디자인 패턴의 아름다움에서도 DDD를 공부하는 것 보다 문제를 해결하려는 영역에 대한 이해가 중요하다고 강조했다.)  
  
서로 다른 의미로 단어를 사용하는 상황을 방지하기 위해 도메인 분석을 통해 유비쿼터스 언어로 소통해야 한다.  
해당 유비쿼터스 언어는 개발자가 구현하는 소프트웨어에도 녹아있어야 한다.  
  
**엔티티**, **값 객체**, **도메인**, **모델**, **서비스**, **모듈**, **집합**에 대해 정의를 내려주며 서로 다른 책임을 가지고 있다면 레이어를 침범해서는 안된다.  
[디자인 패턴의 아름다움 스터디]()에서 팩토리 패턴을 강조하셨었는데 **Aggregate**를 생성할 때 팩토리 패턴을 적극적으로 활용해야 한다고 설명한다.  
