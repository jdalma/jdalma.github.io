...
DDD를 이루는 개념과 원리보다는 도메인(문제를 해결하려는 영역)에 대한 이해를 강조한다.  
(디자인 패턴의 아름다움에서도 DDD를 공부하는 것 보다 문제를 해결하려는 영역에 대한 이해를 강조한다.)  
  
서로 다른 의미로 단어를 사용하는 상황을 방지하기 위해 도메인 분석을 통해 유비쿼터스 언어로 소통해야 한다.  
해당 유비쿼터스 언어는 개발자가 구현하는 소프트웨어에도 녹아있어야 한다.  
  
**엔티티**, **값 객체**, **도메인**, **모델**, **서비스**에 대해 정의를 내려주며 서로 다른 책임을 가지고 있다면 레이어를 침범해서는 안된다.  
