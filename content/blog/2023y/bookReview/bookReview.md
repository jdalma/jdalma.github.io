...
- [모임 내용](https://github.com/jdalma/footprints/blob/main/%EB%94%94%EC%9E%90%EC%9D%B8%ED%8C%A8%ED%84%B4%EC%9D%98%20%EC%95%84%EB%A6%84%EB%8B%A4%EC%9B%80/README.md)
...
칸반 시각화를 통해 계획하지 않은 업무와 문맥 전환으로 발생하는 비용에 대해 준비하는 자세  
"칸반의 목적은 문제를 수면위로 끌어올리는 것이다."  
"칸반은 현재 업무량을 무시하고 업무를 밀어 넣는 대신, 업무를 할 수 있을 때 작업자가 업무를 끌어오는 시각적 당김(풀) 시스템이며 제약조건을 기반으로 한다."  
업무에 대한 일정을 변경하기에는 힘들다. 품질이나 양을 협의하기가 쉽다.  
  
시각-공간 학습자, 다섯 도둑, 린 칸반 흐름  
좋은 시각화? **분산 인지**
"내가 뭘 빠뜨리지 않았나?", "이런것도 해야 되는데" 라는 생각이 들지 않도록 어딘가에 기록을 해야한다.  
현재 진행하는 한 가지의 일에만 집중할 수 있도록 한다.  
