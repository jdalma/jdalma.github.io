...
### 2026-09-22: 도메인 주도 설계 철저 입문

사내 브랜드들이 각각 모놀리식으로 구현되어 있어, 요즘 통합 이커머스 MSA 작업을 진행 중이다. 내가 집중하고 있는 도메인은 주문/결제 도메인이다.  
결제 도메인은 유즈케이스별 끝 지점에 존재하기도 하고, 다른 도메인에 의존하기보다는 의존을 당하는 쪽이라 분리 자체는 어렵지 않았다.  
이제 주문 도메인을 추상화·일반화해서 유즈케이스를 정리하고 API를 설계하려 한다. 이런 작업에 DDD가 도움이 될 것 같아 팀원분에게 여쭤보니 이 책을 빌려주셔서 읽게 됐다.  
  
책 이름에 걸맞게 전술적 설계를 집중적으로 설명한다.  
바이브 코딩 시대에 이런 내용이 의미가 있을까 싶기도 했지만, **모르면 의사결정을 할 수조차 없다.** 구현 부분까지 짚어주어서 쉽게 잘 읽혔다.  
  
DDD 책답게 가장 강조하는 것은 **문제를 정확히 이해하고, 도메인 전문가의 어휘와 시야를 배우려고 노력해야 한다**는 것이다.  
인상깊었던 내용들을 정리해본다.  

1. **값 객체와 불변성**
   - 불변하는 값이 주는 장점
   - 도메인 모델로 선정되지 못한 개념을 값 객체로 정의할지에 대한 기준은 '규칙이 존재하는가'와 '낱개로 다루어야 하는가'
2. **엔티티와 값 객체의 분류 기준**
   - 생애주기의 존재 여부와 그 생애주기의 연속성 여부
   - 같은 대상이어도 값 객체가 될 수도, 엔티티가 될 수도 있다. 어떤 환경에 있느냐에 따라 모델링 방법이 달라진다
3. **도메인 서비스와 애플리케이션 서비스**
   - 입출력에 대한 처리는 가능한 한 도메인 서비스에서 하지 않는 것이 좋다
   - 도메인 객체에서도 입출력을 가능한 한 배제해야 한다. 일급 컬렉션이 이를 관철하는 데 도움이 될 것 같다
4. **응집도와 의존 방향**
   - 응집도를 계산하는 LCOM 방식
   - 추상화 수준이 높은 모듈이 낮은 모듈에 의존해서는 안 된다
5. **인프라를 믿고 검증을 게을리하지 말 것**
   - DB가 제공하는 유일 키 제약을 믿고 애플리케이션, 도메인 레이어에서의 검증을 생략해서는 안 된다
6. **애그리게이트와 리포지토리**
   - 애그리게이트의 경계를 정하는 데 흔히 쓰이는 원칙은 '변경의 단위'다
   - 리포지토리는 애그리게이트 하나씩 만든다
7. **컨텍스트 맵**
   - 컨텍스트가 분리되면서 도메인 엔티티의 변경 사항이 다른 컨텍스트에 전파되지 않는 문제가 생긴다
   - 이를 방지하기 위해 전체 도메인을 내려다볼 수 있는 컨텍스트 맵을 만들어야 한다

다음 DDD 책으로는 조금 더 거시적인 관점에서 DDD를 설명하는 책을 읽어볼 예정이다.  
...
이 내용들을 읽으면서 계속 들었던 생각은 **어떤 방향이나 어떤 생각이 무조건 옳다는 것은 없다**는 것이다.  
