...
최적의 답을 무조건 찾아내지 못 하더라도 (나는 시간복잡도를 확실히 모른다.) 생각하는 개발자라면 기계처럼 코드를 작성하는 것이 아니라 몇 분이라도 고민하고 작성하는것이 중요하다고 생각한다.  
...
8. **맹기완 대표님이 주관하신 [자바에서 코틀린으로 읽기모임](https://github.com/jdalma/java-to-kotlin)** 
   ...
   - *재밌다고해서 마냥 쉽진 않았다* 스터디나 읽기모임에 자주 참여하지만 이렇게 깊게 얘기하고 부담되는 모임은 처음이다.
   - 단순히 책에 나오는 내용들만 보고 넘어가는게 아니라 **객체지향**, **함수형 프로그래밍**, **값 이론** 등에 대한 내용들을 얘기해주셔서 재밌게 완독했다.
...
살면서 종립님을 직접 뵐 기회가 있을까 싶었는데, 코드숨 사무실에 직접 오셔서 강연까지 해주셨다.  
...
SI 회사에서 개발자로 경험할 수 있는 범위는 한정적이라고 생각되었다.  
프로젝트를 빠르게 개발하고 다음 프로젝트에 투입되는 식의 반복으로 개발한 기능에 대한 효과를 알 수 없었다.  

1. 내가 수행한 프로젝트의 영향은 얼마정도일까?
2. 내가 개발한 기능에서 장애가 발생했을까? 발생했다면 원인이 뭐였을까?
3. 기능을 사용하다가 불편한 점은 없었을까?
4. 내가 기능을 요구한대로 완벽하게 개발한걸까?

등 내가 맡은 프로젝트에 개발,운영,피드백을 경험할 수 있는 곳으로 도전하고 싶었다.   
