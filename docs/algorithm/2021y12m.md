...
# **`[LeetCode]` [단뱡항 링크드 리스트 병합](https://leetcode.com/problems/merge-two-sorted-lists/)**

- 정렬된 두 개의 리스트를 병합하여 반환한다.
...
## 👍Best `Time complexity : O(n + m) , Space complexity : O(1)`
...
# **`[백준] 다익스트라` [최소비용 구하기](https://www.acmicpc.net/problem/1916)**
...
# **`[백준] 다익스트라 , BFS` [특정 거리의 도시 찾기](https://www.acmicpc.net/problem/18352)**
...
# **`[백준] 다익스트라` [지름길](https://www.acmicpc.net/problem/1446) - 진행중**
...
class Main {
	static int[][] roadsArr = new int[10001][10001];
	static int[] costArr = new int[10001];
	static final int INF = Integer.MAX_VALUE;
    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        StringTokenizer st = new StringTokenizer(br.readLine());
        int shortCutCount = Integer.parseInt(st.nextToken());
        int destination = Integer.parseInt(st.nextToken());
        int maxDistance = 0;
        for(int i = 0 ; i < 10001 ; i++) {
        	Arrays.fill(roadsArr[i], INF);
        	costArr[i] = INF;
        }
        for(int i = 0 ; i < shortCutCount ; i++) {
        	...
        	int usedCost = Integer.parseInt(st.nextToken());
        	if(roadsArr[startPosition][endPosition] > usedCost) roadsArr[startPosition][endPosition] = usedCost;
        	maxDistance = Math.max(maxDistance , endPosition);
        }
        costArr[0] = 0;
        int position;
        for(position = 0; position < destination ; position++) {
        	for(int j = 0 ; j <= maxDistance ; j++) {
        		System.out.println(position + " " + j + " " + roadsArr[position][j]);
        		if(roadsArr[position][j] != INF && costArr[position] > position + roadsArr[position][j]) {
        			costArr[position] = position + costArr[position];
        			position += roadsArr[position][j];
        		}
        	}
        }
        if(costArr[destination] == INF) {
        	System.out.println(position);
        }
        else System.out.println(costArr[destination]);
    }
...
# **`[백준] 다익스트라` [특정한 최단경로](https://www.acmicpc.net/problem/1504) - 진행중**
