...
# **`백준 - 밸만-포드 알고리즘` [타임머신](https://www.acmicpc.net/problem/11657) - 진행중**
    ...
    public static boolean solution() {
        ...
        for(int i = 1 ; i < edge - 1; i++) {
        	for(Move move : list.get(i)) {
        		...
        		int cost = move.cost;
        		if(costArr[endVertex] > costArr[i] + cost) {
        			costArr[endVertex] = costArr[i] + cost;
        		}
        	}
        }
