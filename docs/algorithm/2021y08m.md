...
# **`[백준 DFS , BFS]` [숨바꼭질2](https://www.acmicpc.net/problem/12851) <span style="color:red; font-weight:bold">시간초과 ➜ 메모리초과</span> ❌**
...
class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        bfsSolution(start , destination);
        dfsSolution(start , destination , 0);
        System.out.println(minTime);
    ...
    public static void bfsSolution(int start , int destination){
        ...
        while(!queue.isEmpty()){
            ...
            if(now.position == destination) {
                ...
                for(int i = 0 ; i < move.length ; i++){
                    int moving = i == 2 ? now.position * move[i] : now.position + move[i];
                    if(now.passedTime + 1 < minTime && moving >= 0 && moving < 100001 && moving < destination + 1 && !visited[moving]){
                        visited[moving] = true;
                        queue.offer(new Moving(moving , now.passedTime + 1));
                    }
                }
    ...
    public static void dfsSolution(int start , int destination , int passedTime){
        if(start < destination + 1 && start < 100001 && passedTime <= minTime){
            for(int i = 0 ; i < move.length ; i++){
                int moving = i == 2 ? start * move[i] : start + move[i];
                if(moving == destination) {
                    ...
                    dfsSolution(moving , destination , passedTime + 1);
                }
            }
        }
    }
