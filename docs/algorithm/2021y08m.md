...
class Main {
    static char[] removeCharArr;
    static char[] charArr;
    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        removeCharArr = br.readLine().toCharArray();
...
//        System.out.println(solution());
    ...
    public static List<Character> solution(){
        ...
        for(int i = 0 ; i < 50 ; i++) resultList.add('0');
        boolean isReverse = false;
        int index = 0;
        int leftIndex = 0  , rightIndex = charArr.length - 1;
        for(char ch : charArr){
            if(index >= charArr.length || index < 0) break;
            if(isReverse) {
                index = isSame(rightIndex);
                if(index != rightIndex) isReverse = false;
                rightIndex = index - 1;
            }
            else {
                index = isSame(leftIndex);
                if(index != leftIndex) isReverse = true;
                leftIndex = index + 1;

            }
            System.out.print(index + " ");
            resultList.set(index , charArr[index]);
        }
        return resultList;
    }

    public static int isSame(int index){
        boolean result = false;
        for(int i = 0 ; i < removeCharArr.length ; i++){
            if(index + i < charArr.length && charArr[index + i] == removeCharArr[i]) result = true;
            else {
                result = false;
                break;
            }
        }
        if(result) return index + removeCharArr.length;
        else return index;
    }
}
```
