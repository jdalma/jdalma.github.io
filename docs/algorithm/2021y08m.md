    ...
    public static void bfsSolution(int start , int destination){
        ...
        while(!queue.isEmpty()){
            ...
            for(int i = 0 ; i < move.length ; i++){
                ...
                else if(now + 1 < minTime && moving >= 0 && moving < 100001 && visited[moving] == 0){
...
## 객체 사용 시 <span style="color:red; font-weight:bold">메모리초과</span>
```java
import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;
...
class Main {
    ...
    public static void bfsSolution(int start , int destination){
        ...
        while(!queue.isEmpty()){
            Moving now = queue.poll();
            if(now.position == destination) {
                if(minTime > now.passedTime){
                    minTime = now.passedTime;
                    minDisCount = 1;
                }
                else if(minTime == now.passedTime){
                    minDisCount++;
                }
            }
            else{
                for(int i = 0 ; i < move.length ; i++){
                    ...
                    if(now.passedTime + 1 < minTime && moving >= 0 && moving < 100001){
                        queue.offer(new Moving(moving , now.passedTime + 1));
                    }
                }
            }
