...
# **`[백준] 그래프 BFS` [적록색약]](https://www.acmicpc.net/problem/10026) 진행 중**
...
class Main {
        ...
        for(int count = 0 ; count < 2 ; count++) {
            ...
            for(int i = 0 ; i < size ; i++) {
            	...
            	for(int j = 0 ; j < size ; j++) {
            		char value = line[j];
            		if(count == 1 && line[j] == 'G') value = 'R';
            		map[i][j] = value;
            	}
            }
            visited = new boolean[size][size];        	
            for(int i = 0 ; i < size ; i++) {
            	char flag = count == 0 ? 'N' : 'Y';
            	for(int j = 0 ; j < size ; j++) {
            		if(!visited[i][j]) {
    ...
    public static void bfs(int x , int y , char color , char flag) {
