...
# **`[백준] 다익스트라` [녹색 옷 입은 애가 젤다지?](https://www.acmicpc.net/problem/4485)**
...
# **`[백준] 다익스트라` [파티](https://www.acmicpc.net/problem/1238)**
...
# **`[백준] 부분 합` [부분 합](https://www.acmicpc.net/problem/1806)** 미완료
...
# **`[백준]` [인간 대포](https://www.acmicpc.net/problem/10473)** 미완료
...
class Position{
	...
	public Position(String x, String y , double wasteTime) {
		this.x = Double.parseDouble(x);
		this.y = Double.parseDouble(y);
	...
	public double getDistance(Position obj) {
		double diffX = Math.abs(this.x - obj.x);
		double diffY = Math.abs(this.y - obj.y);
		return Math.sqrt(Math.pow(diffX, 2) + Math.pow(diffY, 2));
	}
}

class Main {
	static Position start , target;
	static final double RUN = 5;  // 5 m/s
	static final double FLY = 25; // 대포에 올라타고 발사하고 착륙까지 2초 , 50m
    ...
    public static void main(String[] args) throws IOException {
        ...
        target = new Position(line[0], line[1]);
