...
# **`[백준] BFS` [점프 게임](https://www.acmicpc.net/problem/15558) 진행중**
...
class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        bfs();
        if(gameEnd) System.out.println("1");
    ...
    public static void bfs() {
    	...
    	while(!pq.isEmpty()) {
    		...
    		if(gameEnd) break;
    		else if(now.y >= N - 1) gameEnd = true;
    		else {
    			// 한 칸 앞으로
    			if(now.y + 1 < N && map[now.x][now.y + 1] == 1 && !visited[now.x][now.y + 1]) {
    				pq.offer(new Position(now.x , now.y + 1));
    				visited[now.x][now.y + 1] = true;
    			}
    			
    			// 한 칸 뒤로
    			if(now.y - 1 >= 0 && map[now.x][now.y - 1] == 1 && !visited[now.x][now.y - 1]) {
    				pq.offer(new Position(now.x , now.y - 1));
    				visited[now.x][now.y - 1] = true;
    			}
    			
    			// 반대편 줄로 점프 후 K만큼 앞으로
    			int moveX = now.x == 1 ? 0 : 1;
    			int moveY = now.y + K;
    			if(moveY < N && map[moveX][moveY] == 1 && !visited[moveX][moveY]) {
    				pq.offer(new Position(moveX , moveY));
