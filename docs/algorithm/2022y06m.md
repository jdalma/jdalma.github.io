...
class Main{
    ...
    public static long init(int node , int start , int end) {
    	...
    	return tree[node] = init(child , start , mid) * init(child + 1 , mid + 1 , end) % 1_000_000_007L;
    }
    
    public static long update(int node , int start , int end , int index , long diff) {
    	...
    	if(start == end) {
        	...
        	int mid = (start + end) >> 1;
            int child = node << 1;
            return tree[node] = update(child , start , mid , index , diff) * update(child + 1, mid + 1 , end , index , diff) % 1_000_000_007L;
    	}
    ...
    public static long query(int node , int start , int end , int left , int right) {
    	...
    	int mid = (start + end) >> 1;
        int child = node << 1;
    	return query(child , start , mid , left , right) * query(child , mid + 1 , end , left , right) % 1_000_000_007L ;
    }
}
