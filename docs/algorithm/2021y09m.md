...
# **`[백준] BFS` [토마토](https://www.acmicpc.net/problem/7569) 진행중**
...
class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        map = new int[height][ver][hor];
        checked = new boolean[height][ver][hor];
        for(int i = 0 ; i < height ; i++){
            for(int j = 0 ; j < ver ; j++){
                String[] line = br.readLine().split(" ");
                for(int k = 0 ; k < hor ; k++){
    ...
    public static void bfs(int z , int x , int y){
        ...
        while(!queue.isEmpty()){
            ...
            for(int i = 0 ; i < 6 ; i++){
                ...
                if(moveXpos >= 0 && moveYpos >= 0 && moveZpos >= 0 && moveXpos < ver && moveYpos < hor && moveZpos < height){
                    if(map[moveZpos][moveXpos][moveYpos] == 0 && !checked[moveZpos][moveXpos][moveYpos]){
                        map[moveZpos][moveXpos][moveYpos] = 1;
                        checked[moveZpos][moveXpos][moveYpos] = true;
