...
class Solution {
    public int solution(String s) {
       int answer = 0;
        ...
        for(int i = 0 ; i < charArr.length ; i++){
            char[] tmpArr = charArr.clone();
            chkCnt = 0;
            for(int j = 0 ; j < charArr.length ; j++){
                if(tmpArr[j] == 40) close = 41;
                else if(tmpArr[j] == 91) close = 93;
                else if(tmpArr[j] == 123) close = 125;
                else continue;
                for(int k = 0 ; k < charArr.length ; k++){
                    if(j != k && tmpArr[k] == close){
                        if(k - j == 1 || k - j == 3){
                            chkCnt++;
                            tmpArr[j] = '-';
                            tmpArr[k] = '-';
            ...
            for(int j = 1 ; j < charArr.length  ; j++){
                ...
                if( j == charArr.length - 1) charArr[j] = first;
            }
            if(chkCnt == charArr.length / 2) answer++;
        }
//        System.out.println(answer);
        return answer;
    }
