...
class Main {
	...
	static int[][] map;
	static Queue<Position> icebergPos = new ArrayDeque<Position>();
	static int row , col;
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0 ; i < row ; i++) {
			...
			for(int j = 0 ; j < col ; j++) {
				...
				if(map[i][j] != 0) {
					icebergPos.offer(new Position(i , j));
				}
			}
		}
		
		int meltCount = 0;
		while(!icebergPos.isEmpty()) {
			meltCount++;
			melt();
			if(findChunk() > 1) {
				break;
	...
	public static void melt() {
		int icebergSize = icebergPos.size();
		for(int i = 0 ; i < icebergSize ; i++) {
			Position iceberg = icebergPos.poll();
			for(int[] move : moves4dir) {
				if(!isOutOfRange(iceberg , move)) {
	...
	public static boolean isOutOfRange(Position pos , int[] move) {
		int x = pos.x + move[0];
		int y = pos.y + move[1];
