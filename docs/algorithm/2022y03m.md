...
# **`[백준] 이분검색` [풍선공장](https://www.acmicpc.net/problem/15810)**
...
class Main {
	public static void main(String[] args) throws IOException {
		...
		int[] wasteTime = new int[staff];
		
		st = new StringTokenizer(br.readLine());
		for(int i = 0 ; i < staff ; i++) {
			wasteTime[i] = Integer.parseInt(st.nextToken());
		}
		
		Arrays.sort(wasteTime);
		
		long left = wasteTime[0];
		long right = wasteTime[staff - 1] * (long) ballon;
		long mid = 0;
		while(left <= right) {
			mid = (left + right) / 2;
			long makeBallon = 0;
			for(int time : wasteTime) {
				makeBallon += mid / time;
			}
			if(makeBallon < ballon) left = mid + 1; 
			else right = mid - 1;
		}
...
# **`[백준] BFS` [빙산](https://www.acmicpc.net/problem/2573)**
