...
1. 자주 사용하는 것을 자세하게 들여다 봐라.
   - 자바 `util` 패키지를 뜰어보거나 무의식적으로 사용하는 기능들에 대해 공부해라
...
**버전이 높은 만큼 학습해서 적용할 기회는 더 많을 것이다.** 그리고 (인프라에 대해 잘 모르지만) **도커 스웜** ([컨테이너 오케스트레이션](https://seongjin.me/docker-swarm-introduction-nodes/)), **젠킨스**를 사용하여 인프라를 구성하고 있어 관련 기술들을 학습하여 적용할 수 있는 기회도 있을 것이다.  
  
입사 후에 트레이닝 프로젝트와 도메인 교육을 진행했다.  
내가 맡은 트레이닝 프로젝트는 `n차`로 나뉘어 진행되고 있는데  
**1차**는 [Armeria와 gRPC로 통신하기 위한 템플릿 서버 개발](https://jdalma.github.io/docs/retrospective/2022y/2022y12m/#%ED%8A%B8%EB%A0%88%EC%9D%B4%EB%8B%9D-%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8-2)  
**2차**는 1차에서 만든 템플릿 서버로 ES서버에 부하를 주는 검색식 검증 규칙 관리 개발이다. *진행중*  
...
- 이직할 때
  ...
  8. [ ] AWS를 적극적으로 사용하는 회사 → `레거시 프로젝트 서비스 분리 경험` ([마틴 파울러의 마이크로서비스](https://martinfowler.com/articles/microservices.html))
- 학습할 때
  1. [x] 블로그를 꾸준히 운영하기
  2. [x] 학습하는 자세 유지하기
  3. [x] 다른 개발자들과 교류하기
  4. [x] 관심가는 스터디에 망설이지 말고 참여하기
  5. [x] 코드리뷰 받아보기
  6. [x] 테스트 코드 배워보기
  7. [x] 학습에 대한 비용은 아끼지 말기

시간이 지나면서 점점 목표한 것들을 해내고 있다는 생각이 든다.  
**옛날엔 어려워 했던 것을 지금은 아무렇지 않게 해내게 된 것이 가장 많이 변한 것 같다.**  
