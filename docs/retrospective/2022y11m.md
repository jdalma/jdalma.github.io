...
4. 정리가 필요한 패턴
   ...
   - 어댑터 패턴
   - 템플릿 메소드 패턴
5. ⭐️ **`Kotlin` + `Spring Boot` + `gRPC`를 사용한 샘플 프로젝트**
...
- RPC 프레임워크
- **스텁**
  - 매개변수 객체를 메시지로 `변환(마샬링)`/`역변환(언마샬링)`하는 **레이어**
  - 클라이언트의 스텁과 서버의 스텁으로 나눠진다
- **프로토콜 버퍼**
  - 구조화된 데이터를 직렬화하기 위한 구글의 언어 중립적, 플랫폼 중립적, 확장 가능한 메커니즘을 말한다
  - XML이나 JSON 보다 더 빠르고 간단하다
  - 프로토콜 버퍼를 통해 데이터를 주고 받을 수 있어 `어떤 언어나 플랫폼`에서도 통신 프로토콜 등에 쉽게 적용할 수 있다
- **gRPC의 채널**
  - 여러 서브채널을 열어서 통신하며 이 채널을 재사용하여 통신비용을 절약할 수 있다
- **워크플로우**
  1. `Protobufs` 정의
  2. `protoc` 컴파일
  3. 코드 제네레이트
