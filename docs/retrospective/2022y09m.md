...
- 코드숨 알림 기능
  - 강의 알림 신청
    - 사용자 테이블과 1:N 관계를 가져야 한다
    - 강의 테이블과 외래키를 엮어야할까?
    - 강의 id는 화면에서 받을 수 있을 것 같은데 관계를 꼭 잡아줘야할까?
    - 시즌 id도 화면에서 받을 수 있을까?
    - 만약 이 상태로 알림을 보내게 되면 강의 시즌 기준으로 잡아서 돌려야겠는데
    - 전화 번호는 user테이블에 추가할 수 밖에 없을 것 같은데..
    - 시즌에 기간 컬럼이 추가 되어야 할 것 같다
    - 기능 사용자의 요구 사항이 더 필요하다
      - ~~강의 기수가 열렸다가 닫히는 적이 있는지?~~
      - ~~강의 알림 신청이 관리자가 진행하는지 배치로 진행하는지?~~
      - 관리자가 진행한다면 알림을 중복되게 보내고 싶은지?
        - 버튼 누르면 중복 가능
      - ~~마감 알림도 보내고 싶은지?~~
      - ~~오픈 알림과 마감 알림 구분해서 보내고 싶은지?~~
      - ~~내용을 추가적으로 작성하고 싶은지?~~
      - ~~강의와 기수는 가지고 있기 때문에 알림 내용을 템플릿으로 만들 수 있겠지만 관리자가 내용을 더 추가해서 보내고 싶은지?~~
  - 이벤트 메시지 수신 동의
    - user테이블에 플래그 컬럼을 추가하면 되지 않을까?
    - 기존 테이블을 수정하지 않고 이벤트 메시지 수신 동의 테이블을 생성할까?
    - 플래그 컬럼 한 개만 있으면 될까? 더 저장해야할 정보가 있을까?

- **결정 사항**
  - 관리자가 돌린다라는 가정
    - 기수를 알면 코스를 알 수 있다.
    - **알림 테이블** (수신 동의 테이블 , 휴대폰 번호 테이블 , 알림 로그 테이블은 추후에)
      - 개인정보 관련 찾아봐야할듯 
      - 신청 , 신청 취소 , 정상 신청 되었다.
  - **알림 테이블**
    - id , 기수 , 강의 이름 , 번호 , 이메일 , 신청 날짜
- **알림 API**
  - 폼 창 띄울 때 사용자 정보 필요 , 알림 신청 여부
    - 사용자 정보는 기존 API가 있을 듯
    - 알림 신청 여부를 기존 API에서 추가하는 방향을 확인
  - 이메일 , 핸드폰 번호 유효성 검사
  - 정상 등록은 상태 201
...
5. 이번 면접은 기술적으로 깊게 질문 들어온 것이 없었다
   1. 이력서 위주로 물어봄
   2. TDD에 대한 얘기
   3. 관심있는,사용하고 싶은 기술
...
1. 우테캠프로 사전 과제
2. 코드숨 포트폴리오반
3. 스스로 부족하다고 하는 것 채우기
   - 북마크 토이 프로젝트 하기
   - JPA 강의
   - 캐싱 , Redis
우테캠프로 사전과제를 선택했다. (+ 부족한 것 채우기)<br>
내가 선택할 수 있는 것 중에 희소성이 제일 높다.<br>

<br>

윤석님이 생각하시는 완벽한 포르젝트는 아래의 3개가 적절할 때 여러 모습을 보여줄 수 있다<br>
1. 서비스 위주 프로젝트
2. 학습 위주 프로젝트
3. 오픈소스 기여

**뭔가를 공부할 때 너무 전력으로 하려는 문제**<br>
1. 여러 개를 해보면서 버리는 것은 문제가 안된다
2. 나한테 뭐가 중요한지 최소의 노력으로 최대의 결과 만들기
3. 점진적인 개선 중요하다

***

## 우테캠 프로 사전과제 [컨벤션](https://newwisdom.tistory.com/m/96)
