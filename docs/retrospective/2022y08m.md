...
- **코드숨 1주차 과제**
  ...
  5. **외부에서 정보를 꺼내쓰기 보다는 객체 내부에서 판단이 가능하다면 내부에서 처리하게 하자**
  6. 오늘은 정규표현식 찾는 것에 시간을 많이 할애 하였다..
...
## **8월 넷 째주 회고** - 코드숨 4주차 과제 `고양이 장난감 가게 만들기`
...
2. TDD를 통해 **4주차 과제 진행**
   ...
   - 통합 테스트 X
   - `@DataJpaTest`
3. **SpringDataJpa**는 어떻게 엔티티를 업데이트 하는지
...
1. 코드숨 알고리즘 6일차 완료
2. 이펙티브 자바 아이템 3,4,5
3. [`docs oracle` Using Bean Validation Constraints](https://docs.oracle.com/javaee/7/tutorial/bean-validation001.htm)
4. [`Dozer Mapper` Getting Started](https://dozermapper.github.io/gitbook/documentation/gettingstarted.html)
5. 5주차 과제
   - **UserController** 테스트 작성
   - **ServiceController** 테스트 작성
     - UserDTO 필요
     - 직렬화 : id , name , email
     - 역직렬화 : id , name , email , password
   - **E2E 테스트 요구사항에 맞게 작성하자**
6. [Java Language and Virtual Machine Specifications](https://docs.oracle.com/javase/specs/index.html)

***

## **08.30 (화)**
1. 코드숨 알고리즘 7일차 완료
2. `String.class`를 참고하여 **JavaDoc** 수정
3. **CrudRepository 인터페이스**의 `void delete(T entity);`는 해당 엔티티에서 `getID`를 호출하여 삭제한다 
4. **JavaDoc**을 작성할 때는 **빌트인 클래스 `String.class ..`**들을 참고하자
5. [@AllArgsConstructor, @RequiredArgsConstructor 주의](https://jdalma.github.io/docs/algorithmTheory/lombok/#allargsconstructor-requiredargsconstructor-%EC%82%AC%EC%9A%A9%EA%B8%88%EC%A7%80)를 정리했다
6. [`lombok` @EqualsAndHashCode](https://projectlombok.org/features/EqualsAndHashCode)
7. **Controller 통합 테스트**진행
...
1. 코드숨 알고리즘 8일차 완료
2. `@Builder`를 클래스 , 생성자에 작성 차이 [정리](https://jdalma.github.io/docs/algorithmTheory/lombok/#builder-%ED%81%B4%EB%9E%98%EC%8A%A4%EC%97%90-%EC%9E%91%EC%84%B1%ED%95%98%EB%8A%94-%EA%B2%83%EA%B3%BC-%EB%A9%94%EC%84%9C%EB%93%9C%EC%97%90-%EC%9E%91%EC%84%B1%ED%95%98%EB%8A%94-%EA%B2%83%EC%9D%98-%EC%B0%A8%EC%9D%B4%EC%A0%90)
3. [`johngrib` JavaDoc 작성하기](https://johngrib.github.io/wiki/java/javadoc/)
   - **목표는 특정 코드 덩어리의 대략적인 역할을 3초 안에 파악할 수 있도록 도와주는 것이다.**
   - **메소드가 무엇을 입력받아서 무엇을 리턴하는지를 반드시 설명한다.**
   - **구현과 주석이 커플링이 생기지 않도록 한다.**
   - **주석 상속 규칙**
4. **Serivce** 목적에 맞게 분리 [리뷰](https://github.com/CodeSoom/spring-week5-assignment-1/pull/75#discussion_r958434890)
   - `QueryService`
   - `CommandService`
